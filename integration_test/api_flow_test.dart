...
/// Teste de integração Flutter ↔ Backend NestJS rodando localmente.
///
/// Pode ser executado em emulador/dispositivo (base URL vinda de
/// [BackendConfig.apiBaseUrl]) ou no host passando `--dart-define=API_BASE_URL=`.
///
/// Pré-requisitos:
/// 1. Backend rodando acessível a partir do app.
/// 2. Contas de pai e motorista ativadas no banco. Recomenda-se rodar
///    `cd nestjs && bash scripts/e2e-setup.sh` antes e passar as credenciais
///    geradas via `--dart-define`.
///
/// Exemplo de execução em emulador Android:
/// ```bash
/// cd app_faixa_amarela
/// flutter test integration_test/api_flow_test.dart \
///   --dart-define=PARENT_EMAIL=e2e-pai-...@test.faixaamarela.com.br \
///   --dart-define=PARENT_PASSWORD=E2E@Senha123 \
///   --dart-define=DRIVER_EMAIL=e2e-motorista-...@test.faixaamarela.com.br \
///   --dart-define=DRIVER_PASSWORD=E2E@Senha123
/// ```
///
/// Exemplo de execução no host:
/// ```bash
/// cd app_faixa_amarela
/// flutter test integration_test/api_flow_test.dart \
...
void main() {
  ...
  const parentEmail = String.fromEnvironment(
    'PARENT_EMAIL',
    defaultValue: '',
  );
  ...
  group('Flutter ↔ NestJS API integration', () {
    late Dio dio;
    ...
    test(
      ...
      () async {
        ...
        // 1. Busca um turno e uma escola válidos no catálogo.
        ...
        expect(schools, isNotEmpty, reason: 'Catálogo de escolas vazio');
        final school = schools.first;

        // 2. Cria criança vinculada a escola do catálogo.
        ...
        expect(createdChild.schoolId, school.id);
        ...
        final pendingAfter = await parentEnrollmentsRepo.getPendingEnrollments();
        expect(
        ...
        dio.options.headers['Authorization'] =
            'Bearer ${driverSession.accessToken}';
        final driverEnrollments =
            await driverEnrollmentsRepo.getMyEnrollments();
        expect(
    ...
    test('driver profile and planning options are coherent', () async {
      ...
      expect(routes, isA<List<Map<String, dynamic>>>());
    });
    ...
    test('parent routes and boardings endpoints are reachable', () async {
      ...
      expect(routes.items, isA<List<Map<String, dynamic>>>());

      final boardings = await parentRoutingRepo.getBoardings();
      expect(boardings.items, isA<List<Map<String, dynamic>>>());
    });
    ...
    test(
      ...
      () async {
        ...
        final disembarked = await routesRepo.markDisembarking(routeId, childId);
        ...
        try {
          ...
          fail('recalculate failed: ${e.response?.statusCode} ${e.response?.data}');
        }
        ...
        try {
          ...
          fail('telemetry failed: ${e.response?.statusCode} ${e.response?.data}');
        }
        ...
        try {
          ...
          fail('geofence failed: ${e.response?.statusCode} ${e.response?.data}');
        }
    ...
    test('publicities impression and click endpoints are reachable', () async {
      ...
      if (ads.isNotEmpty) {
        ...
        final clickRes = await dio.post<dynamic>('/publicities/$firstId/click');
        expect(clickRes.statusCode, equals(204));
      }
    });

    test('driver profile change request upload-image accepts multipart',
        () async {
      dio.options.headers['Authorization'] =
          'Bearer ${driverSession.accessToken}';
    ...
    test('driver profile change request submit returns pending request',
        () async {
      dio.options.headers['Authorization'] =
          'Bearer ${driverSession.accessToken}';
...
String _generateCpf() {
  ...
  final base = random.toString().padLeft(9, '0').substring(0, 9);
