...
class PusherChannelsFlutterPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    private fun init(
        ...
        try {
            ...
            call.argument<String>("host")?.let { host ->
                tryInvoke(options, "setHost", host)
            }
            call.argument<Int>("wsPort")?.let { wsPort ->
                tryInvoke(options, "setWsPort", wsPort)
            }
            call.argument<Int>("wssPort")?.let { wssPort ->
                tryInvoke(options, "setWssPort", wssPort)
            }
            call.argument<String>("path")?.let { path ->
                tryInvoke(options, "setPath", path)
            }
    ...
    private fun tryInvoke(target: Any, methodName: String, arg: Any) {
        try {
            val method = target::class.java.methods.firstOrNull { method ->
                method.name == methodName && method.parameterTypes.size == 1
            }
            method?.invoke(target, arg)
        } catch (_: Exception) {
            // Ignore unsupported options on older pusher-java-client versions.
        }
    }
